...
describe('extractSecondCases — дополнительная база', () => {
	it('извлекает вопрос + variants + answers + idx из h3 + p > strong', () => {
		...
		const cases = extractSecondCases(div);
	...
	it('возвращает пустой массив если нет вопросов', () => {
		...
		expect(extractSecondCases(div)).toEqual([]);
	});
});

describe('extractFirstCases — основная база, layout1 (h3 + span highlighted)', () => {
	it('извлекает правильные варианты по жёлтому фону', () => {
		...
		const cases = extractFirstCases(div);
...
describe('extractFirstCases — основная база, layout2 (p.MsoNormal + br + плюс)', () => {
...
describe('extractFirstCases — idx уникален в пределах массива', () => {
...
describe('findAnswers — интеграция с extractor-функциями (реальный HTML)', () => {

	it('находит вопрос в результатах extractFirstCases по тексту', () => {
		const div = createDiv(`
		...
		const model = extractFirstCases(div);
		const userVariants = [
...
describe('баг репорт', () => {
	...
	it('Дорсопатия. Миофасциальный болевой синдром: таргетная диагностика и терапия', () => {
		...
		const model = extractSecondCases(div);
	...
	it(' Переломовывих Галеацци – это', () => {
		const div = createDiv(`
		<b>24. Переломовывих Галеацци – это</b>
		<br>				
		
		1) перелом локтевой кости в сочетании с вывихом головки локтевой кости и
		повреждением дистального лучелоктевого сочленения;
		<br>			
		
		2) перелом локтевой кости в сочетании с вывихом головки лучевой кости и
		повреждением проксимального лучелоктевого сочленения;	
		<br>	
		<b>3) перелом лучевой кости в сочетании с вывихом головки локтевой кости и
		повреждением дистального лучелоктевого сочленения;+</b>		
		<br>	
		...
		const userVariants = [
			'перелом лучевой кости в сочетании с вывихом головки локтевой кости и повреждением дистального лучелоктевого сочленения',
			'перелом локтевой кости в сочетании с вывихом головки локтевой кости и повреждением дистального лучелоктевого сочленения',
			'перелом локтевой кости в сочетании с вывихом головки лучевой кости и повреждением проксимального лучелоктевого сочленения',
			'перелом лучевой кости в сочетании с вывихом лучевой локтевой кости и повреждением проксимального лучелоктевого сочленения'
		];
		...
		expect(res).not.toBeNull();
		expect(res?.answers).toHaveLength(1);
		expect(res?.answers?.[0]).toContain('перелом лучевой кости в сочетании с вывихом головки локтевой кости и повреждением дистального лучелоктевого сочленения');
	});

	it('Какое диагностическое исследование не проводится при обследовании пациентов с болезнью Грейвса', () => {
		const div = createDiv(`
		<h3>24. Какое диагностическое исследование не проводится при обследовании пациентов с болезнью Грейвса</h3>
		...
		expect(res).not.toBeNull();
		expect(res?.answers).toHaveLength(1);
		expect(res?.answers?.[0]).toContain('пульсоксиметрия');
	});
